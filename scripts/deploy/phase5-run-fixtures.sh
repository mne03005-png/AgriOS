...
counts="$("${compose[@]}" exec -T agrios-mysql sh -c 'mysql --default-character-set=utf8mb4 -N -B -uroot -p"$MYSQL_ROOT_PASSWORD" agrios -e "SELECT (SELECT COUNT(*) FROM User WHERE id LIKE \"phase5-%\"),(SELECT COUNT(*) FROM Tenant WHERE id LIKE \"phase5-%\");"' 2>/dev/null)"
