...
async function waitForSending(path: string, childProcess: ChildProcess) { let stderr=''; childProcess.stderr?.on('data',(value)=>stderr+=value); for (let attempt=0;attempt<600;attempt++) { if (childProcess.exitCode !== null) throw new Error(`EDGE_EXITED_BEFORE_SENDING:${childProcess.exitCode}:${stderr.trim()}`); try { const state=JSON.parse(await readFile(path,'utf8')); if([...state.telemetry,...state.acks].some((item:any)=>item.status==='SENDING'||item.queueStatus==='SENDING')) return; } catch {} await delay(50); } throw new Error(`EDGE_SENDING_STATE_TIMEOUT_AFTER_30S:${stderr.trim()}`); }
