...
test('reset does not revive a stale queued START', async () => { const a = new ControlCommandArbiter(); let unsafe = 0; const start = a.submit('PUMP_ON', async (dispatch) => { if (dispatch) unsafe++; return dispatch; }); await a.submit('EMERGENCY_STOP', async () => 1); await a.submit('RESET_EMERGENCY_STOP', async () => 1); await start; assert.equal(unsafe, 0); });
test('restored Edge latch blocks START after restart', async () => { const a = new ControlCommandArbiter(); a.restoreEmergencyLatch(true); let unsafe = 0; await a.submit('START_FERTIGATION', async (dispatch) => { if (dispatch) unsafe++; return dispatch; }); assert.equal(unsafe, 0); });
test('expired/reconnected stale START cannot pass active latch', async () => { const a = new ControlCommandArbiter(); a.restoreEmergencyLatch(true); assert.equal(await a.submit('SET_PUMP_FREQUENCY', async (dispatch) => dispatch), false); });
