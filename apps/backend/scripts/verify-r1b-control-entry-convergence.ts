...
test('23 queue retry preserves one action plan identity',async()=>{const f=mobileFixture();const r:any=await f.service.valve({deviceId:'valve-a',command:'VALVE_OPEN'},f.user);const retry={id:r.queueJobId,actionPlanId:r.actionPlanId};assert.equal(retry.actionPlanId,r.actionPlanId);});
