...
import { CoreModbusTcpTransport, isRealPlcWriteEnabled } from '@agrios/edge-core';
...
export class ModbusTcpTransport extends CoreModbusTcpTransport {
  constructor(config: ConfigService) {
    ...
    }), () => isRealPlcWriteEnabled({
      deviceControlMode: config.get<string>('DEVICE_CONTROL_MODE'), deviceControlDryRun: config.get<string>('DEVICE_CONTROL_DRY_RUN'),
      valveAllowRealControl: config.get<string>('VALVE_ALLOW_REAL_CONTROL'), enableAutoExecution: config.get<string>('ENABLE_AUTO_EXECUTION'),
      plcTransport: config.get<string>('PLC_TRANSPORT'), plcRealWriteEnabled: config.get<string>('PLC_REAL_WRITE_ENABLED')
    }));
  }
}
