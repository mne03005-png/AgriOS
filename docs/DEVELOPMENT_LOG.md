# AgriOS Development Log (Internal, P6–P13.2)

This is the pre-existing internal implementation log for the full AgriOS production platform,
moved here verbatim from the repository root `README.md` when that file became the public
OpenAgriOS front door (v0.1-alpha). Nothing below was changed by that release — it documents the
much larger system OpenAgriOS's alpha demo is a small, public slice of. See
[`docs/AUDIT.md`](AUDIT.md) for exactly how the two relate.
...
MVP 的架构边界、首个软件里程碑、数据模型、MQTT 主题、API 与硬件模拟计划见：

- `docs/architecture/AgriOS-MVP-Implementation-Plan.md`
